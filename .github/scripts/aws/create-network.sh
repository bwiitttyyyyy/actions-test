...
aws ec2 create-tags --resources $VPC_ID --tags Key=commit,Value=$GITHUB_SHA --profile production
...
aws ec2 create-tags --resources $SUBNET_ID --tags Key=commit,Value=$GITHUB_SHA --profile production
...
aws ec2 create-tags --resources $GATEWAY_ID --tags Key=commit,Value=$GITHUB_SHA --profile production
...
aws ec2 create-tags --resources $ROUTE_TABLE_ID --tags Key=commit,Value=$GITHUB_SHA --profile production
