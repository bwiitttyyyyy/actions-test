...
# create the VPC
echo "Creating VPC..."
aws ec2 create-vpc --cidr-block 10.0.0.0/16 >> vpc.yml
VPC_ID=$(yq r vpc.yml Vpc.VpcId)
aws ec2 create-tags --resources $VPC_ID --tags Key=commit,Value=$GITHUB_SHA
echo "Created VPC with ID $VPC_ID"

# create subnet
echo "Creating subnet..."
aws ec2 create-subnet --vpc-id $VPC_ID --cidr-block 10.0.0.0/16 >> subnet.yml
SUBNET_ID=$(yq r subnet.yml Subnet.SubnetId)
aws ec2 create-tags --resources $SUBNET_ID --tags Key=commit,Value=$GITHUB_SHA
echo "Created subnet with ID $SUBNET_ID"

# create internet gateway
echo "Creating internet gateway..."
aws ec2 create-internet-gateway >> gateway.yml
GATEWAY_ID=$(yq r gateway.yml InternetGateway.InternetGatewayId)
aws ec2 create-tags --resources $GATEWAY_ID --tags Key=commit,Value=$GITHUB_SHA
...
aws ec2 create-route-table --vpc-id $VPC_ID >> route_table.yml
ROUTE_TABLE_ID=$(yq r route_table.yml RouteTable.RouteTableId)
aws ec2 create-tags --resources $ROUTE_TABLE_ID --tags Key=commit,Value=$GITHUB_SHA
