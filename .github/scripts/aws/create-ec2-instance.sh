...
AWS_DEPLOYMENT_PUBLIC_KEY_ID=$(aws iam list-ssh-public-keys --user-name $AWS_DEPLOYMENT_USERNAME | yq r - SSHPublicKeys.[0].SSHPublicKeyId)
aws iam get-ssh-public-key --user-name $AWS_DEPLOYMENT_USERNAME --ssh-public-key-id $AWS_DEPLOYMENT_PUBLIC_KEY_ID --encoding SSH \
  | yq r - SSHPublicKey.SSHPublicKeyBody >> ssh.pub
echo "WARNING: this may complain about a duplicate key ... this is not an issue"
aws ec2 import-key-pair --key-name $GITHUB_SHA --public-key-material fileb://$PWD/ssh.pub

# create the instance
echo "Creating instance..."
SUBNET_ID=$(yq r subnet.yml Subnet.SubnetId)
aws ec2 run-instances --image-id ami-04a25c39dc7a8aebb --count 1 --instance-type t2.micro --key-name $GITHUB_SHA --subnet $SUBNET_ID >> instance.yml
